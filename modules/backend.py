...
class Backend:
    def __init__(self, tickers, start_prices, current_prices, users, sp500_benchmark) -> None:
    ...
    def __get_prices_stats(self):
        ...
        all_prices = all_prices.rename(
            columns={"name": "Spółka", "yesterday": "Wczoraj", "diff": "Zmiana procentowa"}
        )[["Spółka", "2025-01-02", "Wczoraj", "Zmiana procentowa"]]
        return all_prices.sort_values(by='Zmiana procentowa', ascending=False).reset_index(drop=True)
    ...
    def __get_users_stats(self, prices, users):
        ...
        ranking = ranking.sort_values(by='Średnia', ascending=False).reset_index(drop=True)
        ranking.index += 1
        return ranking 
    ...
    def __get_sp500_benchmark_stats(self):
        ...
        df['Zmiana procentowa'] = round(100 * (df["Wczoraj"] - df["2025-01-02"]) / (df["2025-01-02"] + 1e-6),2)
